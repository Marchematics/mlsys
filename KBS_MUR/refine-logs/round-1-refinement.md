# Round 1 Refinement: Marginal-Utility Routing for Reliable Multi-Context Knowledge-Augmented Prediction
...
缺失的最小机制是：在任意实际选择状态 `A` 上，直接估计候选 `j` 对冻结预测器造成的 counterfactual end-task loss marginal `m(j | A)`，随后重新评分、顺序选择，并用校准区间区分 positive-value selection、certified stopping 与不确定区域。
...
- **Supporting contribution**：建立 estimator error 到 per-state approximate-greedy quality 以及 harm-safe selection / completeness-safe stopping 的校准证书。
...
sequential policy -> select / certified stop / uncertain stop -> update A
```

### Training recipe

1. 在 `expert-train` folds 训练 prediction expert；冻结后，在 disjoint `router-label` folds 生成 targets。正式 real-data run 使用 entity/time-aware cross-fitting，使任何 target 不同时用于拟合 expert 和生成其 router label。
...
初始化 `A=empty`。主部署模型 **MUR-light** 只用缓存 embeddings、set state、cost 和 set size 给剩余候选评分，不执行 `f_{A+j}`。**MUR-full** 加入 batched exact prediction delta，作为 accuracy/cost 上界型变体。达到 budget 或无法认证正效用时停止。

估计误差界产生两个不同证书，必须严格区分：

- **harm-safe selection**：选择最大 LCB；仅当 `LCB>0` 才加入。对 symmetric uniform error，这等价于 `m_hat>epsilon`。
- **completeness-safe stopping**：仅当所有候选 `UCB<=0` 时认证“无 positive remaining marginal”。对 uniform error，这等价于 `max m_hat<=-epsilon`。

当 best candidate 的区间跨零时，系统进入不确定区域。主风险厌恶 policy 选择停止但不称 certified stop；budget-seeking 变体可按 point estimate 继续。论文分别报告 certified-stop rate、uncertain-stop rate 与 gain–harm curve。
...
| Contextual submodular policy | structured reward | Yes | assumes monotone/submodular in core theory | No | task-dependent |
...
5. **Gate N — novelty stress**：所有候选 relation-positive 且 redundant candidates 的 standalone utility 均为正；只有 `A` 改变后边际效用重新排序。若 Set-Coverage 或 Hashimoto incremental 与 MUR 持平，则收窄或停止 novelty claim。
...
- **State distribution shift**：对 random、baseline-prefix、oracle-prefix 与 deployed-prefix states 分层报告误差；只有必要时增加一次 on-policy roll-in，不默认使用迭代 imitation learning。
