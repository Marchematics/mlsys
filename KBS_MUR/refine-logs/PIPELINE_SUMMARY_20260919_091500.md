...
- Working manuscript: `paper/main.tex` and `paper/main.pdf`
...
- The pool-size scaling check now includes $K=4,8,16,32$; the MUR--Static
  gap is small without duplicates and widens under redundancy, while MUR's
  own recovery declines at larger pools.
...
- A same-client-only candidate diagnostic also failed to create a decisive
  margin, so the unresolved issue is the utility signal under the frozen
  prediction expert, not only unrelated-candidate contamination.
