#!/usr/bin/env python3
"""Fixed-set acquisition baselines for the multi-target protocol.
...
def main() -> None:
    ...
    for target_sensor in targets:
        ...
        for seed in seeds:
            ...
            base_test = float(squared_error(test, np.zeros((test.episodes, test.candidate_count), dtype=bool), expert).mean())
            ...
            selections = {
                ...
                "ids_cluster": cluster_representatives(test, relevance, args.budget, seed=seed),
            ...
            for name in AUDIT_POLICIES:
                ...
                metrics[name] = {
                    "mean_prediction_gain": float(base_test - loss.mean()),
                    "negative_transfer_rate": float((loss > base_test).mean()),
                    "mean_utility_recovery": float("nan"),
                    "mean_selected_contexts": float(mask.sum(axis=1).mean()),
                }
            record = {
                "run_id": "R205_fixed_set_baselines",
                "status": "confirmatory",
