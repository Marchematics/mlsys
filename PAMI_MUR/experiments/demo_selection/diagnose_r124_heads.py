#!/usr/bin/env python3
"""R124: three-way head comparison (signed, fixed-sign bilinear, frozen scalar).

Same protocol as the R123 comparison, with the signed-curvature head added as the
primary object under test: for each cell all three heads are trained on identical
full-state data with identical seeds and evaluated on held-out test states.
Also reports the learned signed curvature coefficient ``c_theta`` on those states.
...
from signed_curvature import curvature_summary, train_signed_response_model  # noqa: E402
...
def term_scale_diagnostic(model, ops, batch, states: np.ndarray) -> dict:
    """Within-state spread of the first-order and curvature terms of the signed head."""

    first, curvature, coefficient = [], [], []
    for state in states:
        response = ops.response_summary(batch, state)
        ...
        selected, selected_valid = ops.pack(batch, state, max_budget=state.shape[1])
        query = torch.from_numpy(batch.query_embedding[rows]).float().to(ops.device)
        candidate = torch.from_numpy(batch.candidate_embedding[rows, columns]).float().to(ops.device)
        ...
        features = model._features(query, packed, packed_mask, candidate, cost)
        with torch.no_grad():
            output = model.signed_head(features)
        response_t = torch.from_numpy(response.reshape(-1, response.shape[-1])).float().to(ops.device)
        classes = model.prediction_dim
        d_bar = response_t[:, 7 : 7 + classes]
        kappa = response_t[:, 7 + classes]
        first_term = (output[..., :classes] * d_bar).sum(-1)
        cur_term = output[..., classes] * kappa
        shape = (batch.episodes, batch.candidate_count)
        first.append(first_term.reshape(shape).cpu().numpy())
        curvature.append(cur_term.reshape(shape).cpu().numpy())
        coefficient.append(output[..., classes].reshape(shape).cpu().numpy())
    first_stack = np.stack(first)
    curvature_stack = np.stack(curvature)
    return {
        "within_state_std_first_order": float(np.mean(np.std(first_stack, axis=-1))),
        "within_state_std_curvature": float(np.mean(np.std(curvature_stack, axis=-1))),
        "ratio_curvature_over_first_order": float(
            np.mean(np.std(curvature_stack, axis=-1)) / max(np.mean(np.std(first_stack, axis=-1)), 1e-9)
        ),
        "mean_curvature_coefficient": float(np.mean(np.concatenate([c.reshape(-1) for c in coefficient]))),
    }
...
def main() -> None:
    ...
    for name in [item.strip() for item in args.benchmarks.split(",") if item.strip()]:
        ...
        for seed in seeds:
            ...
            for label, head, head_ops, batch, states in (
                ...
                ("signed", signed_model, bilinear_ops, test_batch, test_states),
            ):
                targets = np.stack(
            ...
            entry["term_scales"] = term_scale_diagnostic(
                signed_model, bilinear_ops, test_batch, test_states
            )
            entry["signed_curvature"] = curvature_summary(
                signed_model, ops, test_batch, states=max(args.states, 2), seed=seed + 41, budget=budget
            )
                ...
                json.dumps(
                    {
                        ...
                        "signed_spearman": round(entry["signed"]["spearman_mean"], 4),
                        "signed_corr_curv": round(entry["signed"]["spearman_with_curvature"], 4),
                        "signed_c_mean": round(entry["signed_curvature"]["mean"], 4),
                        "signed_c_std": round(entry["signed_curvature"]["std"], 4),
                    }
                ),
                flush=True,
            )
        ...
        if entries:
            artifact["cells"][name] = {
                ...
                for label in ("frozen", "bilinear", "signed")
                for key in (
                    "spearman_mean",
                    "top1_agreement",
                    "mean_regret",
    ...
    artifact["summary"] = {
        "mean_signed_spearman": float(np.mean([cell["signed_spearman_mean"] for cell in cells.values()])) if cells else float("nan"),
        "mean_signed_c": float(np.mean([cell["signed_spearman_mean"] for cell in cells.values()])) if cells else float("nan"),
        "benchmarks_signed_spearman_ge_bilinear": int(sum(cell["signed_spearman_mean"] >= cell["bilinear_spearman_mean"] for cell in cells.values())),
