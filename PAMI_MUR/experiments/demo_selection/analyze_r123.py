#!/usr/bin/env python3
"""R123 derived summary: bilinear head vs the frozen scalar head on identical cells.
...
BILINEAR_STEMS = ["ranked_response_q2", "ranked_response_q4", "ranked_response_q8", "ranked_response_full"]
EXTENDED_POLICIES = analyze_r112.POLICIES + FROZEN_STEMS + [stem + "_forced" for stem in FROZEN_STEMS]
...
def paired_head_table(name: str, cells: list[dict]) -> dict:
    """Paired bilinear-vs-frozen contrasts on identical cells."""
    ...
    table = {}
    for bilinear, frozen in zip(BILINEAR_STEMS, FROZEN_STEMS):
...
def main() -> None:
    ...
    for name, aggregate in benchmarks.items():
        for policy, label in (
            ("ranked_response_q4_forced", "bilinear forced"),
            ("ranked_response_q4", "bilinear thresholded"),
            ("ranked_response_frozen_q4_forced", "frozen forced"),
            ("ranked_response_frozen_q4", "frozen thresholded"),
        ):
    ...
    markdown += ["", "### Paired bilinear − frozen head contrast (identical cells)", "",
    ...
    for name, table in head_tables.items():
        for key in ("ranked_response_q4", "ranked_response_q4_forced"):
            ...
            markdown.append(
                ...
                f"[{entry['head_vs_head_hierarchical']['low']:+.4f},{entry['head_vs_head_hierarchical']['high']:+.4f}] |"
            )
    ...
    for label, key, count_key in pooled_rows:
        if key is None:
            values = np.asarray(
                [
                    aggregate["gate"]["ranked_response_frozen_q4_forced"]["gain_over_static"]["mean"]
